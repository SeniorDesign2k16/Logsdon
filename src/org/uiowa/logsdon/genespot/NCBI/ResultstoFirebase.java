...
public class ResultstoFirebase {
	...
	public void SendtoGenespot(ArrayList<Hit> hitInformation, String jobId, String speciesName, String queryNumber,
		...
		if (!speciesHold.equals(speciesName)) {
			...
			cellref.child("JobInformation.Gene").setValue(geneName);
		...
		Firebase newRef = Genespot.child(jobId).child(speciesName).child("Query" + String.valueOf(counter))
				.child(queryNumber);
		 ...
		 * 
		 * accessionNumber.add(hit.getAccessionNumber()); queryStart.add(hit.getQueryFrom());
		 * queryEnd.add(hit.getQueryTo()); hitStart.add(hit.getHitFrom()); hitEnd.add(hit.getHitTo());
		 * frame.add(hit.getHitFrame()); }
		 * 
		 * newRef.child("Accession Number").setValue(accessionNumber); newRef.child("Query Start").setValue(queryStart);
		 * newRef.child("Query End").setValue(queryEnd); newRef.child("JobInformation.Hit Start").setValue(hitStart);
		 * newRef.child("JobInformation.Hit End").setValue(hitEnd); newRef.child("Frame").setValue(frame);
		 * 
		 * counter++;
		 */
	}
